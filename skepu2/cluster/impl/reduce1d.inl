...
namespace skepu2
{
	namespace backend
	{
		...
		::backendDispatch(Size2D size, Iterator arg)
		{
			...
			this->m_mode = skepu2::ReduceMode::ElWise;
			std::vector<skepu2::cluster::starpu_var<T>> partials;
...
#pragma omp declare reduction(UFOMPReducer:T:reducer(omp_in, omp_out))
			...
			if (mode == skepu2::ReduceMode::RowWise) {
...
#pragma omp parallel for reduction(UFOMPReducer: row_res)
			...
			if (mode == skepu2::ReduceMode::ColWise) {
...
#pragma omp parallel for
			...
			if (mode == skepu2::ReduceMode::ElWise) {
