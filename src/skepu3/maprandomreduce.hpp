...
namespace skepu
{
	template<int, int, size_t, typename, typename, typename...>
	class MapRandomReduceImpl;
	
	template<int GivenArity, size_t RandomCount, typename RetType, typename RedType, typename... Args>
	MapRandomReduceImpl<resolve_map_arity<GivenArity, Args...>::value, GivenArity, RandomCount, RetType, RedType, Args...>
	MapRandomReduceWrapper(std::function<RetType(skepu::Random<RandomCount> &, Args...)> map, std::function<RedType(RedType, RedType)> red)
	{
		return MapRandomReduceImpl<resolve_map_arity<GivenArity, Args...>::value, GivenArity, RandomCount, RetType, RedType, Args...>(map, red);
	}
	
	// For function pointers
	template<int GivenArity = SKEPU_UNSET_ARITY, size_t RandomCount, typename RetType, typename RedType, typename... Args>
	auto MapReduce(RetType(*map)(skepu::Random<RandomCount> &, Args...), RedType(*red)(RedType, RedType))
		-> decltype(MapRandomReduceWrapper<GivenArity>((std::function<RetType(skepu::Random<RandomCount> &, Args...)>)map, (std::function<RedType(RedType, RedType)>)red))
	{
		return MapRandomReduceWrapper<GivenArity>((std::function<RetType(skepu::Random<RandomCount> &, Args...)>)map, (std::function<RedType(RedType, RedType)>)red);
	}
	
	// With index
	template<int GivenArity, size_t RandomCount, typename RetType, typename RedType, typename Index, typename... Args, REQUIRES_VALUE(is_skepu_index<Index>)>
	MapRandomReduceImpl<resolve_map_arity<GivenArity, Args...>::value, GivenArity, RandomCount, RetType, RedType, Index, Args...>
	MapRandomReduceWrapper(std::function<RetType(Index, skepu::Random<RandomCount> &, Args...)> map, std::function<RedType(RedType, RedType)> red)
	{
		return MapRandomReduceImpl<resolve_map_arity<GivenArity, Args...>::value, GivenArity, RandomCount, RetType, RedType, Index, Args...>(map, red);
	}
	
	// For function pointers. With index
	template<int GivenArity = SKEPU_UNSET_ARITY, size_t RandomCount, typename Index, typename RetType, typename RedType, typename... Args, REQUIRES_VALUE(is_skepu_index<Index>)>
	auto MapReduce(RetType(*map)(Index, skepu::Random<RandomCount> &, Args...), RedType(*red)(RedType, RedType))
		-> decltype(MapRandomReduceWrapper<GivenArity>((std::function<RetType(Index, skepu::Random<RandomCount> &, Args...)>)map, (std::function<RedType(RedType, RedType)>)red))
	{
		return MapRandomReduceWrapper<GivenArity>((std::function<RetType(Index, skepu::Random<RandomCount> &, Args...)>)map, (std::function<RedType(RedType, RedType)>)red);
	}
	
	// For lambdas and functors
	template<int GivenArity = SKEPU_UNSET_ARITY, typename T1, typename T2>
	auto MapReduce(T1 map, T2 red)
		-> decltype(MapRandomReduceWrapper<GivenArity>(lambda_cast(map), lambda_cast(red)))
	{
		return MapRandomReduceWrapper<GivenArity>(lambda_cast(map), lambda_cast(red));
	}
	
	template<bool indexed, size_t RandomCount, typename RetType, typename... Args>
	struct map_function_type {};
	
	template<size_t RandomCount, typename RetType, typename... Args>
	struct map_function_type<false, RandomCount, RetType, Args...> {
		using type = std::function<RetType(skepu::Random<RandomCount> &, Args...)>;
	};
	
	template<size_t RandomCount, typename RetType, typename Index, typename... Args>
	struct map_function_type<true, RandomCount, RetType, Index, Args...> {
		using type = std::function<RetType(Index, skepu::Random<RandomCount> &, Args...)>;
	};
	
	
	/* MapRandomReduce "semantic guide" for the SkePU 2 precompiler.
	 ...
	 */
	template<int InArity, int GivenArity, size_t RandomCount, typename RetType, typename RedType, typename... Args>
	class MapRandomReduceImpl: public SeqSkeletonBase
	{
		using MapFunc = typename map_function_type<is_indexed<Args...>::value, RandomCount, RetType, Args...>::type;
		...
		RetType apply(pack_indices<OI...>, pack_indices<EI...>, pack_indices<AI...>, pack_indices<CI...>, size_t size, CallArgs&&... args)
		{
			if (disjunction((get<EI>(args...).size() < size)...))
				SKEPU_ERROR("MapRandomReduce: Non-matching container sizes");
	...
	private:
		MapFunc mapFunc;
		RedFunc redFunc;
		MapRandomReduceImpl(MapFunc map, RedFunc red): mapFunc(map), redFunc(red) {}
		...
		friend MapRandomReduceImpl<InArity, GivenArity, RandomCount, RetType, RedType, Args...> MapRandomReduceWrapper<GivenArity, RandomCount, RetType, RedType, Args...>(MapFunc, RedFunc);
		
	}; // end class MapRandomReduce
